    ...
    List.fold
      (fun (((ops, prev_state, budget), tx)
         ...
         | Some
             ...
             (let tx_balance_in_tez =
                1mutez * abs tx_balance in
              if tx_balance > 0
              then
                ...
                (match (budget - tx_balance_in_tez) with
                 ...
                 | Some (diff) ->
                     ( // checks the size of the bound dataif Bytes.
                         length
                           bound_data
                         <> 0n
                      then
                        failwith
                          "UNEXPECTED_EMPTY_BOUND_DATA"
                      else ops, new_sapling_state, diff))))
      tx_list
      (([] : operation list), s, Tezos.get_amount ()) in
