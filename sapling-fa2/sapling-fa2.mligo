...
let main (tx_list, s : parameter * storage) : return =
  ...
  then failwith "UNEXPECTED_XTZ_AMOUNT"
    ...
    let (unshielding_reqs, shielding_amount, new_storage) =
      List.fold
        (fun (((unshielding_reqs, shielding_amount, storage),
             tx)
            : loop_els) -> match Tezos.sapling_verify_update
               tx
               storage.state
           with
             None -> failwith "INVALID_SAPLING_TX"
           | Some
               (bound_data, (tx_balance, new_sapling_state)) ->
               ( // If the balance is strictly positive (i.e. unshielding), we send// funds to the given address.

                if tx_balance > 0
                then
                  (match (Bytes.unpack bound_data
                      : key_hash option)
                   with
                     None ->
                   ...
                   | Some (recipient_key_hash) ->
                       let recipient =
                         ...
                         |> Tezos.address in
                       let data =
                         (recipient, abs (tx_balance)) in
                       (data :: unshielding_reqs,
                        shielding_amount,
                        {
                          storage with
                            state = new_sapling_state
                        }))
                else
                  if tx_balance < 0
                  then
    ...
    let op : operation =
      match ((Tezos.get_entrypoint_opt
            "%transfer"
            s.fa2_contract)
         : fa2_parameter contract option)
      with
        None -> failwith "%TRANFER_DOESNT_EXIST"
      | Some contract ->
          ( // transfers to the contractlet transfers_to
           : fa2_parameter =
             [
               {
