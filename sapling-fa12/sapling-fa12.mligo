...
let main (tx_list, s : parameter * storage) : return =
  ...
  then failwith "UNEXPECTED_XTZ_AMOUNT"
    ...
    match ((Tezos.get_entrypoint_opt
          "%transfer"
          s.fa1_2_contract)
       : fa1_2_parameter contract option)
    with
      None -> failwith "INVALID_FA1_2_CONTRACT"
    | Some contract ->
        let (ops, new_state) =
          List.fold
            (fun (((ops, new_state), tx)
                : (operation list * 8 sapling_state) *
                  8 sapling_transaction) -> match Tezos.
                      ...
                      (match (Bytes.unpack bound_data
                       ...
                       | None ->
                           ...
                           then
                             ...
                             new_sapling_state)))
            tx_list
            (([] : operation list), s.state) in
        (ops, {s with state = new_state})
